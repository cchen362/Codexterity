...
One line each. Once the governed code exists, its `D-` marker is authoritative and the reasoning lives at the anchor. **Every decision below is now anchored in real, shipped code** — the injector core, the palette tooling, the emitted theme, and (from Phase 4 M1) the theme loader. Grep the anchor before proposing a change to the behaviour it governs.
