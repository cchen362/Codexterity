...
6. **Re-probe `pre, code, kbd, samp` on a screen containing code.** Still zero matches on the empty state — untested, not refuted. Much less load-bearing than it was: code surfaces are reachable through stage-1 tokens (`--color-background-editor-opaque`, `--color-token-text-code-block-background`) with no element selector at all.
7. **Launcher: detect a running Codex instance.** *New, found 2026-08-01.* Codex is single-instance; launching it while an instance runs prints `Opening in existing browser session`, hands off and exits, so **the injector silently does not attach**. The user gets an unthemed app and no error. The launcher must detect and report this.
