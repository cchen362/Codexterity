...
**THE ONE THING OUTSTANDING: the terminal panel's code face.** `font-family` inherits and the run-4 check inspected only the element itself, so an **inline style on an ancestor** was never ruled out. The instrument now reports `fontOrigin=` — the highest ancestor sharing the computed face, with its tag, class, inline `font-family` and both mono variables — which answers it on the next launch. **Do not add a landmark before that line is read.** Codex's whole CSS bundle contains only two monospace declarations and *neither* can explain the panel (findings §8.6.2). Phase 3 does not close until this is measured; **Phase 4 is gated behind it and has not started.** **Dialogs** remain the only surface never observed; Codex's settings screen carries no `[role=dialog]` and may simply not have one.
