...
**Depends on:** [`docs/research/phase1-research-findings.md`](../research/phase1-research-findings.md) (the evidence) and [`docs/research/gate0-findings.md`](../research/gate0-findings.md) (the first observations of the running app — where the two disagree, Gate 0 wins).
**Decision state:** D-0001-1 (injection) and D-0001-5 (naming) **accepted**; D-0001-1 needs an amendment recording that the working API is `executeJavaScript` style-tag injection, not `insertCSS`, which is **broken on this Electron fork** — the no-debug-port guarantee is intact and the CDP fallback was **not** needed. D-0001-6 (flat surfaces) and D-0001-8 (rejected directions) **accepted 2026-07-31**. **D-0001-7 is now split: the ground/palette half stands; the TYPOGRAPHY half is REOPENED by the owner 2026-08-01** on readability grounds after seeing Fraunces in the running app — see [`DECISIONS.md`](../DECISIONS.md) §Open. D-0001-9 (hero artwork) and D-0001-10 (character pass) **accepted 2026-08-01**. D-0001-2/3/4 remain owner-pending.
...
1. **Rebuild the hook and token inventory from the running app.** All four declared landmarks matched nothing in the live DOM, and the token-gap probe was invalid (it sampled *after* our own injection, so it could not tell our properties from Codex's). Needs a probe that runs **before** injection, or that walks specific elements and reports which `var()` references resolve their computed values. Until this lands, the real reach of Layer 1 is unknown and [`customizable-ui-inventory.md`](../specs/customizable-ui-inventory.md) §Tier 2 must be treated as stale.
2. **Re-derive Layer 2 against real landmarks.** The character pass the owner approved is currently **inert** — it paints nothing beyond `::selection` and `scrollbar-color`. It degrades correctly rather than breaking, but the approved treatment is not visible.
3. **Resolve the typography reopening.** Build a rendered comparison of Fraunces-for-display paired with candidate *text* faces, judged at real UI sizes; do not choose in prose. See [`DECISIONS.md`](../DECISIONS.md) §Open.
4. **Close the multi-accent violation.** The empty-state card icons render stock blue/purple/green/red in the real app — four accents where the design floor permits one. Fixable only once (1) identifies what paints them.
5. **Amend D-0001-1** to record `executeJavaScript` style-tag injection as the working primary API, with the author-origin and DOM-node caveats already noted in `injector/core/inject.js`.
6. **Re-probe `pre, code, kbd, samp` on a screen containing code.** It matched nothing on the empty state, where no code exists — that is expected, not evidence of absence.
