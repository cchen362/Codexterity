...
**Dialogs** remain the only surface never observed; Codex's settings screen carries no `[role=dialog]` and may simply not have one — report opportunistically, do not build a run for it.
**The ACTIVE plan is now [Plan 0002 — Phase 7: QA, update resilience, docs, release](0002-phase-7-qa-docs-release.md)** (opened 2026-08-04). This document stays the architecture authority and the Phase 1–4 record; Phase 7 execution, including the correction of this file's own stale status text and of the Phase 7 gate in §11, is owned there.
...
| **4. Packaging** ✅ **COMPLETE 2026-08-03** | `.ccskin` format + injector CLI (apply/restore/verify) + safe-CSS validation | ✅ M1–M4 done. Codex launched through the argument-free `cdx`, the real 681,124-byte `.ccskin` loaded and painted, and `restore` left nothing residual (D-0001-24/25) |
| **5. Windows Installer** ✅ **COMPLETE 2026-08-03 (M4)** | Launcher + MSIX exe resolver + installer + shortcuts | ✅ Installed from the built artifact the way a recipient would; the Start-menu shortcut launches themed Codex with **no console window**, and uninstall leaves nothing residual. The MSIX-redirection failure this exposed is **D-0001-29**. *(The original gate also said "survives a simulated app-version bump"; the version range is declarative and deliberately unenforced — that ruling and its simulated-degradation proof belong to Phase 7, [Plan 0002](0002-phase-7-qa-docs-release.md) M4.)* |
| **6. macOS Installer** ✅ **COMPLETE 2026-08-03 (M4)** — built, unverified, and that is its finished state | Wrapper `.app` + `.dmg`, handed to the collaborator | ✅ **Installer built, and its unverified status documented.** *(Gate CHANGED 2026-08-02 — see the D-0001-16 amendment. It was "friend confirms theme applies on macOS"; no milestone may depend on another person's willingness to run it. The collaborator may never install it, and that is an accepted outcome, not an open task.)* |
| **7. Testing & Release** | Cross-platform QA, update-resilience test, docs, release | **Windows verified end to end from the built artifact** — build → install → launch → theme applies in both modes → `cdx restore` → stock → uninstall → nothing residual. **macOS built and documented unverified**, per the D-0001-16 amendment. **No gate here may depend on another person running anything.** Update resilience is proved by simulating the failure (landmarks that cannot match), not by waiting for a Codex update. Owned by [Plan 0002](0002-phase-7-qa-docs-release.md) |
