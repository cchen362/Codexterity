...
10. **Sidebar hover/active rows — VERIFIED, no code needed.** `--color-token-list-hover-background` → `--vscode-list-hoverBackground` → **`--color-background-button-secondary-hover`**, a stage-1 token this theme already defines. Measured on the active row: `#E7DECC`, our value. A plan to re-derive the interaction steps was **dropped after tracing the chain** — the ramp is not broken and widening a step in an owner-approved palette to fix a non-problem was the wrong instinct.
11. **Empty-state card hairlines and composer send control — NOT BUILT, no measured hook.** The control census hit its 120-element cap inside the sidebar and never reached the main content, and the screen probed was a conversation, which has no empty-state cards. Cap raised to 400; both need one census run **on the empty state**. Two guessed selectors were deliberately not shipped.
...
13. **Code face reopened by the owner 2026-08-02.** Monaspace Xenon renders correctly (`font="Monaspace Xenon"`, not a fallback) but reads oddly, because Xenon is the **slab-serif** Monaspace and the app is then serif at every level — display, UI and code. The owner asked about Codex's original mono, IBM Plex Sans and Work Sans. **Unresolved; needs a render before any change** (see D-0001-7's precedent).
