...
Living specs: `docs/specs/` (customizable-ui-inventory, css-architecture, asset-manifest). Evidence base: `docs/research/phase1-research-findings.md`. **No plan is open.** The most recent, [`docs/plans/0005-package-identity-cdp-injection.md`](plans/0005-package-identity-cdp-injection.md), **closed 2026-09-26 with all three milestones done** and shipped as tag **`v0.3.0`** ([notes](releases/v0.3.0.md)): Codex `26.924` requires Windows package identity to start, which killed the direct-launch route, so Windows now launches Codex through `Invoke-CommandInDesktopPackage` and injects over loopback CDP (D-0005-1). Both themes are verified through the installed shortcut on that Codex. The plan before it, [`docs/plans/0004-codex-owl-token-remap.md`](plans/0004-codex-owl-token-remap.md), **closed 2026-09-23 with all four milestones done**. Codex `26.917` had moved onto its "OWL" runtime and renamed the hooks the theme hangs on, so the theme rendered stock. Plan 0004 re-pointed the engine at the new names. **Both themes are owner-verified on OWL Codex** through the installed shortcut, in both modes, including ChatGPT Chat/Work, a terminal and a diff. It shipped as tag **`v0.2.0`** ([notes](releases/v0.2.0.md)), the build for OWL Codex; `v0.1.0` stays the unmaintained build for pre-OWL Codex (D-0004-3). Its measured map of the new token layer is [`docs/research/owl-token-inventory.md`](research/owl-token-inventory.md). The plan before it, `docs/plans/0003-image-led-theme-authoring.md` (the theme-authoring pipeline), closed 2026-08-05; `docs/plans/0001-captains-cabin-architecture.md` is the architecture authority and the Phase 1–4 record.
...
- **Injector** (`injector/`) — shared Node core, with **two entry points and one set of styling logic**. **On Windows since Plan 0005 (2026-09-26): `injector/attach-cdp.js`**, a separate Node process the launcher runs after starting Codex; it connects to a loopback Chrome DevTools Protocol endpoint on a random per-launch port and applies the theme to every Codex `app://` page (D-0005-1). This is the *only* route on Codex `26.924`+, which requires Windows package identity to start and does not pass a launcher's environment through to an identity-launched process. **On macOS: `injector/core/preload.js`** via `NODE_OPTIONS=--require`, the original D-0001-1 mechanism, unverified (D-0001-16). Both routes share the theme loader, the style-tag script, the root-environment report and the landmark verdicts; the CDP route presents a page session through a `webContents`-shaped adapter so the diagnostics run unchanged. Re-applies on navigation; verifies declared DOM landmarks and degrades gracefully. **The "built but not default CDP fallback" older documents describe never existed in code** — it was designed and recorded, and first written in Plan 0005.
...
> **THE SECOND UPDATE IT DID NOT SURVIVE UNCHANGED — and this time the LAUNCH ROUTE broke, not the vocabulary. Codex `26.924.1866.0`, measured 2026-09-26 ([Plan 0005](plans/0005-package-identity-cdp-injection.md)).** Codex's new startup requires the process to carry a **Windows package identity**, which only a Windows-activated Store app has. Codexterity had always started `ChatGPT.exe` directly as a child process so `NODE_OPTIONS` could reach it; that process has no identity, so Codex's own bootstrap failed before any of our code ran ("ChatGPT failed to start. The process has no package identity").
>
> - **What held:** the theme packages, the loader, the `.ccskin` format, the emitter, the OWL vocabulary from Plan 0004, the landmark reporting and the `cdx` CLI. **No palette value and no token name changed.**
> - **What broke:** the mechanism itself, on Windows. A direct launch fails with and without the injector attached (proved with `-NoTheme`). `Invoke-CommandInDesktopPackage` starts Codex with identity but passes **no environment** through, so the preload route cannot ride it; it does pass **arguments**, and Codex honours `--remote-debugging-port`. So Windows now injects over loopback CDP (D-0005-1), the route D-0001-1 had reserved as a fallback but which had never actually been written.
> - **What it cost:** a new attacher process (`injector/attach-cdp.js`), a launcher re-route, and giving up "no debug port ever": while a themed Codex runs, a DevTools endpoint is open on `127.0.0.1` on a random port. The owner accepted that on 2026-09-26.
> - **The lesson for the next update:** `injector.log` at **0 bytes** together with `Desktop bootstrap failed to start the main app` in `launcher.log` means the **launch route** broke, not the theme. Plan 0004's rule still holds for the other case: `injected OK` plus present landmarks with a stock-looking app means renamed names.
