...
**Since Plan 0003 M3 there is a step BEFORE the recipe: the hero-image recommender (D-0003-2).** Hand [`tools/palette/recommend-palette.mjs`](../tools/palette/recommend-palette.mjs) an image and it proposes the *recipe inputs* that suit it — a ground hue and chroma, an accent hue — and nothing else. **It never proposes a final hex**, so every value still goes through the OKLCH engine; and it runs `audit.mjs`'s own checks over each proposal *before returning it*, so a caller cannot obtain an unaudited one. It reads a picture as two regions: the **dark mass names the ground**, and **what glows in it names the accent** — a whole-image average is a broken instrument here, because a warm-and-cold image cancels itself to grey (measured: Captain's Cabin's own hero averages to chroma 0.0036). An accent hue must also clear 40° from every semantic status hue, the separation `ROLE` already ships between brass and warning; a violating hue is *projected* and both numbers reported.

```bash
node tools/palette/recommend-palette.mjs <image.png> --inherit-from navy
node tools/mockup/build-palette-recommendation.mjs   # the rendered review sheet
```

The sheet it writes, `docs/mockups/0006-palette-recommendation.html`, is **generated and deliberately not tracked** (D-0003-3) — it embeds the hero it is judging, and those source photographs are private. Rebuild it rather than looking for it in a clone.
...
**Two properties of the emitter that are load-bearing, not incidental.** (1) It **refuses to write anything** if either mode fails WCAG AA — the audit runs inside the emitter, before a byte reaches disk, so a theme is *structurally unable* to ship while failing AA. (2) It **refuses to write anything** if a recipe's landmark `probe` is not present in the stylesheet it just generated (D-0001-21). Both gates run before the single write site, so a failed build leaves no half-emitted theme behind. **Note what `272/272` does and does not prove:** `audit.mjs` sweeps a registry of *grounds* (navy and oak), and 136 of those 272 checks cover a ground no theme ships — it is an **engine**-level proof. A given theme's proof is the emitter's own refusal, which audits exactly that recipe's two palettes and reports the count when it runs.
