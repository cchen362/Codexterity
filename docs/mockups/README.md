...
| `0006-palette-recommendation.html` | 2026-08-05 | **Generated, not tracked — the one build product in this directory (D-0003-3).** The review sheet for [Plan 0003](../plans/0003-image-led-theme-authoring.md) M3's hero-image palette recommender: what the instrument measures in an image, the recipe inputs it proposes, and every proposal rendered as a dark and light UI mock with its audit count. It embeds the hero it is judging, and for the portrait that hero is one of the owner's private photographs, so committing the sheet would put the photograph into git history. Rebuild it with `node tools/mockup/build-palette-recommendation.mjs`. |
