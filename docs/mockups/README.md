...
| `0005-jisoo-palette-directions.html` | 2026-08-04 | **Untracked as of 2026-08-05 (D-0003-10) — it embeds the private photograph, so it is no longer in the repository and this row is the only record left.** It was committed and pushed the day before the rules that would have caught it; the copy already in git history is knowingly left there. Design exploration for the next portrait-led theme: four dark/light palette directions using the shipped typography roles and character treatment. **Decided 2026-08-05:** the owner chose **Deep Navy Portrait**, and the theme was subsequently built and installed — see [Plan 0003](../plans/0003-image-led-theme-authoring.md), which CLOSED the same day. Read this sheet as the record of the decision, not of what shipped: the light mode it renders was superseded twice over. This sheet proposed re-deriving light cooler; the owner then picked the **shipped warm parchment** on sight at the M3b chooser, so Deep Navy Portrait's palette is **identical to Captain's Cabin's** and only the hero and its two solved scrims differ. The other three directions are rejected. |
| `0007-palette-directions.html` | 2026-08-05 | **Generated, not tracked — the one build product in this directory (D-0003-3).** The chooser for [Plan 0003](../plans/0003-image-led-theme-authoring.md) M3b: several complete palette **directions**, each named, each carrying a one-line intent, each rendered as a large dark and light UI mock — plus a light-mode comparison for Deep Navy Portrait. Deliberately carries **no** engineering figures; those live in `node tools/palette/directions.mjs --image <hero>`. It embeds the heroes it is judging, one of which is a private photograph, so committing it would put that photograph into git history. Rebuild with `node tools/mockup/build-palette-directions.mjs`. **Supersedes `0006-palette-recommendation.html`**, which was built and retired the same day: it reported what an instrument measured rather than offering a choice, and the owner's verdict was that its figures *"doesn't help or serve any purpose for me visually"*. |
